...
defParams = SortedDict(dict(
    ...
    noise_bw = 0.0,#1500.0,
    noise_f0 = 0.0,#200.0,
    mode1 = dict(amp=1.0,  envrisetime=5.0, tsnorm=1.0/32, tfnorm=25.0/32, toffnorm=1.0/16, f0N=0.7 , f1N=0.8, noise_std=0.5*np.pi, noise_bwN=0.2, nmode=1),
    mode2 = dict(amp=1.0, envrisetime=5.0, tsnorm=3.0/16 , tfnorm=15.0/16  , toffnorm=1.0/8 , f0N=0.55, f1N=0.7, noise_std=0.5*np.pi, noise_bwN=0.2, nmode=2),
    levels=[1e-6,7e-5,.005],#3,
    #mode1 = dict(amp=1.0,  envrisetime=1.0, tsnorm=1.0/16, tfnorm=13.0/16, toffnorm=1.0/16, f0N=0.35 , f1=0.9, noise_std=0.5*np.pi, noise_bw=0.2, nmode=1),
    #mode2 = dict(amp=1.0, envrisetime=1.0, tsnorm=1.0/8 , tfnorm=7.0/8  , toffnorm=1.0/8 , f0N=0.2, f1=0.8, noise_std=0.5*np.pi, noise_bw=0.2, nmode=2),
...
for mode in [mode1,mode2]:
    for fkey in ['f0','f1','noise_bw']:  mode[fkey] = mode.pop(fkey+'N')*fN
...
sythdataname='synth1'
brnch=root['OUTPUTS'].addBranchPath(f"['{sythdataname}']['data']")
synthholder,arrayoutputs=(b[0] for b in  brnch)
arrayoutputs.insert(0,'parameters',defParams)
arrayoutputs.addBranchPath("['stft']")
arrayoutputs.addBranchPath("['sp']")
dataoutputs=arrayoutputs['data']
stftoutputs=arrayoutputs['stft']
spoutputs=arrayoutputs['sp']
dataoutputs['time'] = t
dataoutputs['sigarr'] = sigarray
stftoutputs['stftarr'] = sigstft
stftoutputs['freq'] = fstft
stftoutputs['time'] = fstft
spoutputs['sp0'] = sp0
spoutputs['freq0'] = fsp0
spoutputs['time0'] = tsp0
spoutputs['sp'] = sp
spoutputs['freq'] = fsp
spoutputs['time'] = tsp
...
if colors is None:
    ...
    cdict['colors'] = colors
#cntrcbnumberformat='%0.2e'
cntrcbnumberformat='%0.1e'
formatter=matplotlib.ticker.FormatStrFormatter(cntrcbnumberformat)
...
if True: #ticks at bases of colors
    ...
    cb.set_ticks(np.linspace(.5,ncolors-0.5,ncolors)/ncolors,labels=[cntrcbnumberformat%l for l in levels])
