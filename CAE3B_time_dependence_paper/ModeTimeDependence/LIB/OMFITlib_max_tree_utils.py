...
#    dictionary value is the level value of the child.
    ...
    for iS in S[::-1]:
        ...
        if image_rav[ipar] != image_rav[iS]: # canoncical non-root node
            if numchldcanon_rav[ipar] == 1: Plevel[level_rav[chld[ipar]]]=ipar
            if numchldcanon_rav[ipar] >= 1: Plevel[currlevel]=ipar
            chld[ipar]=iS
...
#this one attempts to process all "columns" in parallel, which is slow
    if value is None: value = np.ones(P.shape,dtype=np.float64)
    if weight is None: weight=np.ones(P.shape,dtype=np.int)
...
#    is the same-index-parent, and then accumulates from the value at the start to the same index-parent.
    if value is None: value = np.ones(P.shape,dtype=np.float64)
    if weight is None: weight=np.ones(P.shape,dtype=int)
    if value.ndim  > 2:
...
class first_at_index_accumlator:
    ...
    def accumulate(self,image_node,branch):
        ...
        if not self._keyinaccumulation(key): self.accumulation[key] = image_node
...
#this is can easily crash by recursing too deep for modest image sizes.
    ...
    def _mark(inode,image_rav,marked_rav,P_rav,S,branch=(0,)):
        if not accumulator is None and hasattr(accumulator,'accumulate'): accumulator.accumulate(inode,branch)
...
def parent_tree_along_axis_alt1(P,S,axis=0):
    ...
    for p in S[::-1]:
        ...
        while pp != p:
            ...
            if ilaaiiip >= 0: Pa_rav[ilaaiiip] = p
            ilastatind[iip] = p
            p = pp
            pp = P_rav[p]
        iip = ii[p]
        if ilastatind[iip] >= 0: Pa_rav[ilastatind[iip]] = p
    return Pa
