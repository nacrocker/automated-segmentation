...
def imshowxy(x, y, img, xlabel=None, ylabel=None, title=None, ax=plt, **kwargs):
    im = ax.imshow(
        img, aspect='auto', origin='lower', extent=(np.min(x.ravel()), np.max(x.ravel()), np.min(y.ravel()), np.max(y.ravel())), **kwargs
    )
    if xlabel:
        plt.ylabel(ylabel)
    if ylabel:
...
def specim(time, freq, spec, ax=plt, **kwargs):
    ...
    print(kwargs.keys())
    im, cb = imshowxy(time, freq, spec, ax=ax, **kwargs)
    return im, cb
